...
impl McServerImpl {
    pub async fn new(
        ...
        url: &str,
        group_id: i64,
        server_type: BotResult<McServerType>,
    ) -> BotResult<()> {
        match self.select_server_by_name_group_id(name, group_id).await {
            ...
            Some(_) => Err(BotError::from("本群已有该服务器的简称喵...")),
        }
    }

    pub async fn select_server_by_name_group_id(
        &self,
        name: &str,
        group_id: i64,
    ) -> Option<McServer> {
        let mc_server = McServer::select_server_by_name(pool!(), name, group_id)
            .await
            .ok()?;
        mc_server
    }
    ...
    pub async fn select_server_all_by_group_id(&self, group_id: i64) -> Option<Vec<McServer>> {
        let mc_server = McServer::select_by_column(pool!(), "group_id", group_id)
            .await
            .ok();
        mc_server
    }

    pub async fn update_name_by_name_group_id(
        &self,
        name: &str,
        group_id: i64,
        new_name: &str,
        ...
        match self.select_server_by_name_group_id(name, group_id).await {
            None => Err(BotError::from("本群并没有绑定这个服务器简称喵...")),
            Some(mc_server) => {
                McServer::update_by_column(
                    pool!(),
                    &McServer {
    ...
    pub async fn update_url_by_name_group_id(
        &self,
        name: &str,
        group_id: i64,
        new_url: &str,
    ...
    pub async fn update_server_type_by_name_group_id(
        &self,
        name: &str,
        group_id: i64,
        new_server_type: BotResult<McServerType>,
    ) -> BotResult<()> {
        match self.select_server_by_name_group_id(name, group_id).await {
            None => Err(BotError::from("本群并没有绑定这个服务器简称喵...")),
            Some(mc_server) => {
